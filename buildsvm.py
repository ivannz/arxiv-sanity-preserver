...
if not os.path.isfile(Config.database_path):
    print(
        "the database file as.db should exist. You can create an empty database with sqlite3 as.db < schema.sql"
    )
    sys.exit()
...
for ii, u in enumerate(users):
    print(
        "%d/%d building an SVM for %s" % (ii, len(users), u["username"].encode("utf-8"))
    )
    uid = u["user_id"]
    ...
    sortix = np.argsort(-s)
    sortix = sortix[
        : min(num_recommendations, len(sortix))

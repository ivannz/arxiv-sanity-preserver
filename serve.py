...
def recommend():
    ...
    ctx = default_context(
        ...
        msg="Recommended papers: (based on SVM trained on tfidf of"
        " papers in your library, refreshed every day or so)"
        if g.user
...
def library():
    ...
    if g.user:
        ...
        msg = (
            "You must be logged in. Once you are, you can save papers to"
            " your library (with the save icon on the right of each paper)"
...
def friends():
    ...
    if not g.user:
        ...
        if len(papers) == 0:
            msg = (
                "No friend papers present. Try to extend the time"
                " range, or add friends by clicking on your account"
                " name (top, right)"
            )
        else:
